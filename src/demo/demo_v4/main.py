...
# demo_v4 is at src/demo/demo_v4/ = 4 levels from root
...
def main():
    ...
    while True:
        ...
        # ── Expand tracker IR / macro format if needed ──
        ...
        if is_tracker_ir(result):
            ...
            log("info", f"Expanding Tracker IR ({sections} sections, {patterns} patterns → flat tracks)...")
            try:
            ...
            log("info", f"Expanding V2 macro ({len(result.get('patterns',{}))} patterns → flat tracks)...")
            try:
